...
def test_openai_compat_post_chat_accepts_responses_api_shape(monkeypatch):
    class FakeClient:
        async def post(self, url, headers, json, timeout):
            return httpx.Response(200, json={"output_text": "OK from responses"})
    ...
    result = asyncio.run(
        OpenAICompatClient(
            ...
            {"model": "test-model", "messages": [{"role": "user", "content": "Reply OK"}]},
            "check",
        )
    )
...
def test_openai_compat_post_chat_uses_reasoning_content_when_content_empty(monkeypatch):
    class FakeClient:
        async def post(self, url, headers, json, timeout):
            return httpx.Response(200, json={"choices": [{"message": {"content": "", "reasoning_content": "OK from reasoning"}}]})
    ...
    assert result == "OK from reasoning"


def test_openai_compat_unknown_response_shape_logs_parse_error(monkeypatch, caplog):
    class FakeClient:
        async def post(self, url, headers, json, timeout):
    ...
    with pytest.raises(RuntimeError, match="không có nội dung assistant"):
        asyncio.run(
            OpenAICompatClient(
                Settings(_env_file=None, openai_compat_base_url="https://compat.test/v1", openai_compat_text_model="test-model", openai_compat_api_key="secret")
            )._post_chat(
    ...
    assert "AI provider parse error provider=openai_compat kind=check model=test-model" in caplog.text
    assert "empty_content_or_unknown_shape" in caplog.text


def test_openai_compat_local_parse_error_name_is_bound(monkeypatch):
    import app.services.openai_compat_client as openai_compat_client

    captured = {}

    def fake_parse_logger(provider, kind, model, message, response_chars=None, limit=500):
        captured.update({
            "provider": provider,
            "kind": kind,
            "model": model,
    ...
    monkeypatch.setattr("app.services.provider_logging.log_provider_parse_error", fake_parse_logger)

    openai_compat_client.log_provider_parse_error("openai_compat", "scene", "test-model", "invalid_json", response_chars=10)
...
def test_openai_compat_parse_error_logging_guard_does_not_mask_error(monkeypatch, caplog):
    from app.services.openai_compat_client import _log_openai_compat_parse_error

    def broken_logger(*args, **kwargs):
        raise NameError("log_provider_parse_error")

    monkeypatch.setattr("app.services.provider_logging.log_provider_parse_error", broken_logger)
    caplog.set_level(logging.WARNING, logger="app.services.ai_providers")

    _log_openai_compat_parse_error("scene", "test-model", "invalid_json", response_chars=10)

    assert "AI provider parse error logging failed provider=openai_compat kind=scene model=test-model" in caplog.text
