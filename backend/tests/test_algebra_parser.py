...
def test_normalizer_converts_latex_calculus_templates():
    ...
    assert normalize_algebra_input(r"\int_1^z\left(2x\right)dx=2") == "integral(expr=(2*x),var=x,a=1,b=z,target=2)"
    assert normalize_algebra_input(r"\int_1^2 2xdx") == "integral(expr=2*x,var=x,a=1,b=2)"
